...
class DoctorRegistrationScreen(ctk.CTkFrame):
    def __init__(self, parent, app):
        ...
        # Title Section
        create_title_label(form_container, "Doctor Registration", self.colors).pack(pady=(0, 10))
        create_subtitle_label(form_container, "Create your professional account", self.colors).pack(pady=(0, 30))
        ...
        _, self.doctor_id = create_label_entry(form, "Doctor ID (D-XXXX)", "D-654321", colors=self.colors)
        _, self.name = create_label_entry(form, "Full Name", "Dr. Robert Smith", colors=self.colors)
        _, self.specialization = create_label_entry(form, "Specialization", "e.g., General Practitioner", colors=self.colors)
